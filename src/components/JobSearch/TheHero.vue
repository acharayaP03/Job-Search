    ...
    <spot-lights class="flex flex-row justify-center pb-16">
      <template #default="{image, title, description, index}">
        <router-link
          v-if="index !== 1"
          to="/jobs/results"
          class="mx-5 flex flex h-[268px] w-[380px] flex-col overflow-hidden rounded-lg border bg-brand-gray-2"
        >
          <img :src="image" :alt="title" class="h-2/4 w-full object-cover" />
          <div class="mt-3 px-6 py-2">
            <h3>{{ title }}</h3>
            <p class="mt-3 truncate text-sm">{{ description }}</p>
          </div>
        ...
        <router-link
          v-if="index === 1"
          to="/jobs/results"
          class="mx-5 flex h-[268px] w-[530px] flex-col overflow-hidden rounded-lg border bg-brand-gray-2"
        >
          <div
